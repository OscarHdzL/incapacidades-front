...
export class IncapacidadComponent extends ParentComponent implements OnInit {

	@Input() idee: string;
	@Output() edadDetalleEvent: EventEmitter<number> = new EventEmitter<number>();

	public pageRequest: PageRequest<FiltroIncapacidad> = new PageRequest<FiltroIncapacidad>();
	...
	public modelIncapacidad: ModelIncapacidad = new ModelIncapacidad();
	public filtroIncapacidad: FiltroIncapacidad = new FiltroIncapacidad();
	...
	public arraySubTable = [
		...
		{ field: 'diasAutorizados', header: 'Días autorizados', subHeader: '', enabled: true },
		{ field: 'diasProbables', header: 'Días probables de recuperación', subHeader: '', enabled: true },
		{ field: 'diasAcumulados', header: 'Días acumulados', subHeader: '', enabled: true },
		{ field: 'ramoSeguro', header: 'Ramo de seguro', subHeader: '', enabled: true },
		{ field: 'tipoIncapacidad', header: 'Tipo de incapacidad', subHeader: '', enabled: true }
	];
	...
	public selectedDates(event) {
		...
		if (this.filtroIncapacidad.fecha.start && this.filtroIncapacidad.fecha.end) {
			this.filtroIncapacidad.fecha.start = '';
			this.filtroIncapacidad.fecha.end = '';
		}
		if (!this.filtroIncapacidad.fecha.start) {
			this.filtroIncapacidad.fecha.start = (day + '/' + month + '/' + year);
			return;
		}
		if (!this.filtroIncapacidad.fecha.end) {
			this.filtroIncapacidad.fecha.end = (day + '/' + month + '/' + year);
	...
	public clearDate() {
		this.filtroIncapacidad.fecha = undefined;
		this.getFilter();
		this.getDiagnosticos();
		this.getResultadosIncapacidades(1);
	}
	...
	public clearIfNotEndDate(){
		if(this.filtroIncapacidad?.fecha && (this.filtroIncapacidad.fecha.end == '')){
			this.filtroIncapacidad.fecha = undefined;
	...
	private async getOoads(): Promise<any> {
		...
		return new Promise((resolve) => {
			this.incapacidadService.getDatosPorFiltro(this.modelIncapacidad, ID_TIPO_CATALOGO.ooads).subscribe(response => {
	...
	private getFilter() {
		...
		this.filtroIncapacidad.idee = this.idee;
		...
		this.modelIncapacidad.model = this.filtroIncapacidad;
	}
	...
	public getDiagnosticos() {
		...
		this.spinner.show();
		return new Promise((resolve) => {
			this.incapacidadService.getDatosPorFiltro(this.modelIncapacidad, ID_TIPO_CATALOGO.diagnosticos).subscribe(response => {
				this.diagnosticos = response.body.content;
				this.spinner.hide();
				// this.getResultadosIncapacidades(1);
				resolve(response);
			});
	...
	public ooadChange() {
		...
		return new Promise((resolve) => {
			this.incapacidadService.getDatosPorFiltro(this.modelIncapacidad, ID_TIPO_CATALOGO.unidades).subscribe(response => {
	...
	public async getResultadosIncapacidades(page: number): Promise<any> {
		...
		return new Promise((resolve) => {
			...
			this.incapacidadService.getResultadosIncapacidades(this.pageRequest)
				.subscribe(response => {
					this.spinner.hide();

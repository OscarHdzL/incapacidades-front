...
export class ReferenciaService extends HttpService {

  constructor(private http: HttpClient, protected spinner: NgxSpinnerService, private modalDialogService: ModalDialogService) {
		super();
	}

  public getReferencias(pageRequest: PageRequest<FiltroReferencia>): Observable<any> {
		return this.http.post<any>(ENDPOINTS.referencia.referencias, pageRequest, { observe: 'response', headers: this.httpHeadersAnon }).pipe(
			(response: any) => {
				return response;
			},
